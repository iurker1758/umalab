...
- **Choice:** `backend/app/data/cards.json` (card_id → character name +
  ...
  manual, deliberate act on game updates. The API key is a script input
  (`--api-key-file` or env var), and `uma_api_key.txt` is gitignored as a
  guard. (v1 briefly bundled a `factor_names.json` copied from the
