...
## 32. Watched sparks: one store, a hunting bit, and user-named groups

Issue #39, built ahead of the two features that consume it. No UI: a
client store and its API, so #28's chooser and #27's watched block can
be built against the same list instead of each inventing one.
...
- **Requirements:** three features ask the same question — *which
  ...
  #27's watched block wants the ones you are breeding FOR, uncapped and
  including the ones no ancestor carries. Hunted-skill spark scoring
  (the next milestone) wants that second set again. Those two sets
  genuinely differ: a filler white you put on every node is worth
  saving to type and worth nothing to highlight. And a user hunting a
  Front Runner build this week and a Medium build next needs both
  around without one polluting the other's reading.

- **Choice: one store, `umalab.sparks.watched`, holding
  `{kind, key, hunting, groups?}[]` in insertion order.** `kind`/`key`
  is the identity — never a name, which is a localized string resolved
  at render (#30) — one entry per pair, matching the uniqueness rule
  the `factors` document already uses. The two distinctions the
  requirements name are carried on the row, not by splitting the list:
  `hunting` separates "keep this handy to type" from "I want this
  outcome", and `groups` holds the user's own build names. A spark that
  belongs to two builds is one row in two groups, which is a thing
  separate lists can only express by duplicating it — and a duplicate
  is a row you can edit in one place and not the other. `list(group?)`
  filters the one list; `groupNames()` derives the vocabulary from the
  rows, so a group exists exactly as long as a spark is in it and there
  is no second thing to keep in sync.

  **New entries default to `hunting: true`.** Adding is a deliberate
  two-step pick — the spark, then its star level — which reads as "I
  want this outcome". The alternative leaves #27's block empty on first
  use, which reads as broken, and the bit that fills it is one the user
  has not seen yet. Filler is one click off at the moment you add it.
  ...
  **No reconcile pass**, unlike `reconcileFilters`. A watched spark
  ...
  reference is regenerated by hand and can run behind a dump, and #30
  already rules that unknown keys are accepted rather than rejected.
  The asymmetry is deliberate: a filter naming a spark no veteran
  carries hides the roster with nothing on screen to explain why, and a
  watched spark nobody carries is *the most useful row on #27's block*.

  **Its own module** (`frontend/src/sparks.ts`) rather than domain.ts
  or filters.ts. Both existing store pairs sit beside the thing they
  configure — `SORT_STORE` beside `sortVeterans`, `FILTER_STORE` beside
  `matchesFilters` — and this configures neither the roster grid nor
  its filters; it is read by designer surfaces. Shelving it in
  filters.ts would also invite a reconcile pass by proximity.
  ...
  **The roster-mark idiom does not fit**, and it is worth writing down
  ...
  `trained_chara_id`, served by a tag API, and the set only changes
  when the game adds one. Watched sparks are an **open, user-authored
  set over a 432-entry reference**, keyed by `(kind, key)`, belonging
  to the browser and to no row on the server. They share the word
  "favourite" and nothing else; reusing the mark machinery would mean a
  migration every time a user invents a group name.

- **Ships without tests, on purpose.** The repo has no unit runner and
  #30 rejected adding one: `aptitude.ts`'s comparably load-bearing
  arithmetic is verified through the e2e suite against the rendered UI,
  and a second precedent for the *smaller* module would leave the older
  one looking untested. With no UI, the e2e suite cannot reach this
  until #28 lands, and #28's checks are its first coverage. One gap
  those checks will not close by themselves: the loader's
  garbage-fallback path, the only branch here that can fail
  destructively — a throw on load takes the designer tab with it. #28
  should seed the key with a junk value via `addInitScript` and assert
  the chooser still opens empty.

- **Alternatives rejected:** *three list-shaped stores split by intent*
  (favourites / hunting / scoring) — the app already carries four
  persisted client stores and exactly one favouriting idiom; three more
  that differ only in intent is how "which of my lists is this in?"
  becomes a question the user holds in their head. It also cannot
  express a spark in two builds without duplicating it, and the set of
  builds a user cares about is not fixed at three. *One undifferentiated
  list* — real, and rejected for the opposite reason: it collapses
  "quick to type" into "hunting this", which sends every filler white
  into #27's uncapped block and drowns the rows that block exists for.
  The distinction is right; it is just a bit on a row rather than a
  list. *Server-side* — a preference, not a document. *A `stars` field
  on the entry* — the list records which sparks you care about, not
  what level you last typed; #28's chooser carries the level into the
  slot document, which is where a level means something.

- **What would change my mind:** hunted-skill scoring wanting to record
  the list a run was scored against would make this server-side — that
  migration is a table and an endpoint, not a reshape, so it stays
  cheap while the shape holds. Groups proving to be the primary axis
  ...
  rather than in each caller's argument. And if `hunting: true` by
  default fills #27's block with filler in practice, the flip is one
  constant and no stored data changes meaning.
