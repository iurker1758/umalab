...
class SparkList(Base):
    """One of this user's named spark lists (DECISIONS.md #37, issue #39).

    A build's worth of sparks under a name the user chose — "Front Runner",
    "Medium". Several features read them: the chooser's Favorites section
    shows the union, #27's watched block and hunted-skill scoring read
    whichever lists are active at the time. Which those are is a device-local
    view choice and lives in the browser, not here.

    The list is the object with a lifecycle — created empty, renamed,
    deleted — which is why membership hangs off it rather than sitting as a
    bit or a label on a spark row. #33 had that the other way round and could
    not represent an empty list, a rename, or two lists at once; #37 holds
    why.

    Membership is rows in `spark_list_members`, one per (list, kind, key),
    written one at a time through their own routes — two devices editing one
    list commute instead of overwriting each other's whole array
    (DECISIONS.md #48, issue #66).
    """
    ...
    # `onupdate` fires on an ORM flush of THIS row, not on writes to the
    # member table — the membership routes bump it explicitly.
    updated_at: Mapped[datetime] = mapped_column(
        server_default=func.now(), onupdate=func.now()
    )


class SparkListMember(Base):
    """One spark in one list (DECISIONS.md #48, issue #66).

    A row per (list, kind, key) rather than a JSONB array on the list: adds
    and removes are single-row writes with their own routes, so concurrent
    edits to one list commute instead of last-write-wins over a whole array
    computed from whatever copy the client held.

    `id` is the display order — members render in the order they were added,
    the role the array's order used to play (`spark_lists.id` carries
    creation order the same way). A re-add lands ON CONFLICT DO NOTHING and
    keeps its place; removed and re-added, it moves to the end, which is
    what the old client-side filter-and-append did too.

    (kind, key) is the identity, never a name — names are localized strings
    resolved at render time (DECISIONS.md #30). Unknown keys are accepted for
    the same reason blueprint factors accept them: `app/data` is regenerated
    by hand and can run behind a dump, and a spark missing from the reference
    is still a legitimate thing to want. There is no reconcile pass anywhere.

    The CHECK on `kind` must stay in sync with `ListSparkKind` in
    app/schemas.py. It is what lets `SparkListOut` stay strict on read
    without the all-or-nothing failure the old column had (issue #66's
    absorbed #75): a row the model cannot parse is unrepresentable, not a
    500 on every list.
    """

    __tablename__ = "spark_list_members"
    __table_args__ = (
        UniqueConstraint("list_id", "kind", "key", name="uq_spark_list_member"),
        CheckConstraint(
    ...
    list_id: Mapped[int] = mapped_column(
        ForeignKey("spark_lists.id", ondelete="CASCADE"), index=True
    )
