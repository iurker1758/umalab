...
class SparkList(Base):
    ...
    deleted — which is why membership is an array ON it rather than a bit or
    a label on a spark row. #33 had that the other way round and could not
    represent an empty list, a rename, or two lists at once; #37 holds why.

    `sparks` is a JSONB array of `{"kind": str, "key": int}` in the order the
    user added them, deduped by the schema. (kind, key) is the identity,
    never a name — names are localized strings resolved at render time
    (DECISIONS.md #30). Unknown keys are accepted for the same reason
    blueprint factors accept them: `app/data` is regenerated by hand and can
    run behind a dump, and a spark missing from the reference is still a
    legitimate thing to want. There is no reconcile pass anywhere.

    A membership change rewrites the whole array, so concurrent edits to one
    list are last-write-wins — tolerated rather than solved, see #37.
    """
