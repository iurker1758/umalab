...
class WatchedSparkIn(BaseModel):
    ...
    **Both fields are required.** The PUT is a full replace, so a default
    here is not a convenience — it is a silent delete: `{"hunting": false}`
    would erase every build label on the row, with no error and no way to
    tell it happened. The "new sparks are hunted" default (DECISIONS.md #33)
    belongs to whoever is *adding* one, and lives in the client.
