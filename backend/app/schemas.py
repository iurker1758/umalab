...
class SparkListPatch(BaseModel):
    ...
    **Every field is optional and omitting one leaves it alone** — absent and
    ...
    # Bounded because it is written straight into a Postgres int4: an
    # unbounded int lets `{"position": 3000000000}` past Pydantic and turns a
    # 422 into a 500 on commit, which contradicts this module being the
    # authority on what the server accepts. The ceiling is far above any real
    # ordering — 50 lists is the per-owner cap — and leaves room for a
    # reorder UI that spreads positions rather than renumbering.
