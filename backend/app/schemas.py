...
class WatchedSparkIn(BaseModel):
    ...
    `hunting` defaults TRUE (DECISIONS.md #33): adding one is a deliberate
    pick — the spark, then its star level — which reads as "I want this
    outcome", and defaulting false would leave the watched block empty on
    first use with the bit that fills it one the user has never seen.
    ...
    hunting: bool = True
    # Duplicates collapse rather than 422 — the client sends the row's whole
    # group set, and "already in that group" is the state it asked for.
    groups: list[WatchedGroup] = Field(default_factory=list, max_length=20)

    @field_validator("groups")
    @classmethod
    def _dedupe(cls, groups: list[str]) -> list[str]:
