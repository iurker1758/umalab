...
class SlotFactorIn(BaseModel):
    ...
    skill — and a stored row this model rejects reads back raw, which the
    client refuses to open. Rejecting keys here would turn a reference gap
    into a saved design nobody can open.
    """
...
class BlueprintSlotIn(BaseModel):
    ...
    Every rule in this model runs on READ too — BlueprintOut tries
    BlueprintSlotsIn first — so a stored row that fails a rule here reads
    back as raw JSON the client refuses to open (issue #92). A rule that
    must bind on write only, leaving old rows openable, goes in
...
class BlueprintIn(BaseModel):
    ...
    def _validate(self) -> BlueprintIn:
        ...
        # save again.
        #
        # Checked HERE and not in BlueprintSlotIn, because BlueprintSlotIn
        # runs on read too — the slot model must stay permissive so a row
        # saved before this rule stays openable (DECISIONS.md #39). This is
        # not the unknown-key leniency of #30: no reference lookup is
        # involved, the key contradicts the slot itself.
...
class BlueprintOut(BaseModel):
    id: int
    name: str
    # A stored document the strict model can't parse comes back as its raw
    # JSON instead of failing the response — the client's own parse decides
    # what to do with that one row (issue #92, DECISIONS.md #51). left_to_right
    # is load-bearing: the default smart union strict-matches a plain dict
    # against JsonValue first, which would skip strict parsing (and its
    # normalization) for every row, not just bad ones. JsonValue rather than
    # dict because nothing constrains the JSONB column's shape server-side.
    slots: BlueprintSlotsIn | JsonValue = Field(union_mode="left_to_right")
