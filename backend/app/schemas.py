...
class WatchedSparkIn(BaseModel):
    ...
    **Both fields are optional, and omitting one leaves it alone** (issue
    #64 — DECISIONS.md #33's amendment holds the reasoning). Absent and
    `null` alike; `groups: []` is how you clear them, an explicit empty list
    being a different request from a missing key. That distinction is what
    the shape rests on.

    Defaults for a row being *created* live on the columns
    (`models.WatchedSpark`), so "new sparks are hunted" has one owner.

    `extra="forbid"` because absence now carries meaning: with both fields
    optional, a mis-keyed body (`{"hunted": false}`) would otherwise parse
    as all-absent and answer 200 having written nothing, so a failed save
    would reach the user as a successful one.
