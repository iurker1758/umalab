...
def read_stored(stored: Any) -> BlueprintOut:
    """A stored JSONB document arriving at BlueprintOut the way the list
    route serializes it — with no write-path validation in front."""
    now = dt.datetime(2026, 7, 31, 12, 0, 0, tzinfo=dt.UTC)
    return BlueprintOut.model_validate(
        {"id": 1, "name": "stored", "slots": stored, "created_at": now, "updated_at": now}
    )


def out_of(body: BlueprintIn, stored: dict[str, Any] | None = None) -> BlueprintSlotsIn:
    ...
    BlueprintOut. Returns the slots, asserting the strict branch parsed them;
    raw passthrough is the lenient-read section's territory."""
    out = read_stored(body.slots.model_dump() if stored is None else stored)
    assert isinstance(out.slots, BlueprintSlotsIn)
    return out.slots
...
def test_pulled_document_round_trips_through_blueprint_out() -> None:
    ...
    out = out_of(body)
    assert out == body.slots
    assert out.sparks[0] == PinkSparkIn(aptitude="dirt", stars=2, card_id=100601)
    named3 = out.named[3]
...
def test_v2_document_round_trips_through_blueprint_out() -> None:
    ...
    assert out.sparks[11] == PinkSparkIn(aptitude="dirt", stars=2)
...
def test_factors_default_to_empty_so_older_rows_parse() -> None:
    ...
    # as a member carrying none rather than an unopenable raw row.
...
def test_a_pre_rule_mismatched_green_still_reads() -> None:
    # The rule is write-only (DECISIONS.md #39): the STRICT branch must keep
    # parsing a row saved before it landed — falling back to raw would make
    # that document unopenable, not just unsaveable.
...
def test_unknown_spark_key_accepted() -> None:
    ...
    # reference gap into a saved blueprint nobody can open.
...
# ---------- lenient read (issue #92) ----------


def partial_aptitudes_slots() -> dict[str, Any]:
    partial = letters()
    del partial["long"]
    stored: dict[str, Any] = doc(named={1: roster_slot(aptitudes=partial)})["slots"]
    return stored
...
@pytest.mark.parametrize(
    "stored",
    [
        # Wrong-length arrays — the document is positional.
        {"named": [None] * 3, "sparks": []},
        # Half the envelope missing entirely.
        {"sparks": [None] * 24},
        # A slot rule failing inside a well-formed envelope.
        partial_aptitudes_slots(),
        # Not even an object.
        ["not", "a", "document"],
    ],
)
def test_a_document_the_strict_model_rejects_reads_back_raw(stored: Any) -> None:
    # One malformed row must not fail the whole list response (issue #92).
    # It comes back verbatim — not repaired, not dropped: a repaired row
    # would save over data the fix guessed at, and a dropped one reads as
    # deleted, which the designer answers by creating a fresh blueprint.
    out = read_stored(stored)
    assert not isinstance(out.slots, BlueprintSlotsIn)
    assert out.slots == stored


def test_a_valid_stored_document_still_parses_strict() -> None:
    # The union is left_to_right on purpose: smart mode matches a plain dict
    # against JsonValue first, which would hand every row back raw and drop
    # the read-side normalization older rows rely on.
    out = read_stored(doc(named=FULL_TRIANGLE)["slots"])
    assert isinstance(out.slots, BlueprintSlotsIn)
